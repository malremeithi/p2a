...
struct proc *hold_lottery(int total_tickets)
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    ...
    {
      if(p->boostsleft==0)
        tick_count += p->tickets;
      else
         tick_count += 2*p->tickets;
      // winner_ticket_number = p->tickets;
      if (winner_ticket_number <= tick_count)
      {
        release(&ptable.lock);
        return p;
      }
...
//  r never returns.  It loops, doing:
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
      if (p->state == RUNNABLE)
      {
        if (p->boostsleft!=0)
        {
          ...
          tot += p->tickets;
        // p->boostsleft+=1;
        // settickets(p->pid, 2*(p->tickets));
        // cprintf("pid: %d, %s, tickets:  %d\n", p->pid,p->name, p->tickets);
      }
      if(p->state == SLEEPING){
        p->boostsleft+=1;
      }
      // tot += p->tickets;
      // cprintf("pid: %d, %s, tickets:  %d", p->pid,p->name, p->tickets);
      // continue;
    }
    release(&ptable.lock);
    if (tot > 0)
    {
      ...
      struct proc *p = hold_lottery(tot);
      acquire(&ptable.lock);
      c->proc = p;
      p->inuse = 1;
...
int getpinfo(struct pstat *var)
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if (p->state == UNUSED || p->state == ZOMBIE || p->state == EMBRYO)
    {
      ...
      var->inuse[index] = 1;
    var->pid[index] = p->pid;
    var->tickets[index] = p->tickets;
    var->runticks[index] = p->runticks;
    var->boostsleft[index] = p->boostsleft;
    index++;
  }
  return 0;
}
